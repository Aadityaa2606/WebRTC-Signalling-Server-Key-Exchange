...
if (
  !fs.existsSync("server_public.pem") ||
  !fs.existsSync("server_private.pem")
) {
  let [serverPublicKey, serverPrivateKey] = kyber.KeyGen768();
  fs.writeFileSync(
    "server_public.pem",
    serverPublicKey.toString("base64"),
    "utf8"
  );
  fs.writeFileSync(
    "server_private.pem",
    serverPrivateKey.toString("base64"),
    "utf8"
  );
}
...
const serverPublicKey = Buffer.from(
  fs.readFileSync("server_public.pem", "utf8"),
  "base64"
);
const serverPrivateKey = Buffer.from(
  fs.readFileSync("server_private.pem", "utf8"),
  "base64"
);

// Load Clients' Public Keys
const clientAPublicKey = Buffer.from(
  fs.readFileSync("../client_A/client_a_public.pem", "utf8"),
  "base64"
);
const clientBPublicKey = Buffer.from(
  fs.readFileSync("../client_B/client_b_public.pem", "utf8"),
  "base64"
);

// Logging function
const logTime = (messageLength, times) => {
  const logEntry = `${messageLength},${times.join(",")},${(
    times.reduce((a, b) => a + b) / times.length
  ).toFixed(3)}\n`;
  fs.appendFileSync("time_logs.csv", logEntry);
};
...
io.on("connection", (socket) => {
  ...
  socket.on("sendSharedSecret", (encryptedSharedSecret, callback) => {
    const startTime = performance.now(); // Start measuring time

    try {
      // Decrypt the shared secret with the server's private key
      let [sharedSecret] = kyber.Decrypt768(
      ...
      let [encryptedSharedSecretForClientB] = kyber.Encrypt768(
        clientBPublicKey,
        sharedSecret
      );
      ...
      socket.broadcast.emit(
        "receiveSharedSecret",
        encryptedSharedSecretForClientB
      );

      const endTime = performance.now(); // End measuring time
      console.log(
        `Time taken for key exchange: ${(endTime - startTime).toFixed(3)} ms`
      );

      callback(null, (endTime - startTime).toFixed(3)); // Return time to client for logging
    } catch (error) {
  ...
  socket.on("sendMessage", (encryptedMessage, callback) => {
    try {
      ...
      const [encryptedMessageForClientB] = kyber.Encrypt768(
        clientBPublicKey,
        message
      );
